...
def rainbow_print(data, display_mode=DisplayMode.HIGHLIGHT, text_color=TextColor.YELLOW,
                  background=BackgroundColor.BLUE, file=sys.stdout):
    """
    ...
    """
    print(base_str.format(display_mode.value, text_color.value, background.value, data), file=file)
...
def print_rainbow(file=sys.stdout):
    ...
    for tc in t:
        for bc in b:
            print('\033[1;{};{}m{} [textcolor:{}, background:{}]\033[0m'.format(tc, bc, 'RainBow', TextColor(tc).name,
                                                                                BackgroundColor(bc).name), file=file)
...
def print_table(table, title, theme=Table_Theme.RED, border_style=BorderStyle.DEFAULT, rich_mode=False, hilight=[],
    ...
    for index, row in enumerate(table):
        ...
        if index == len(table) - 1:
            ...
            print_row(row, theme.value[1], hilight=hilight, border_style=border_style, alignment=alignment)
            print_spilt_line(start=border_style['split_line_start'], mid=border_style['split_line_mid'],
                             end=border_style['split_line_end'], line=border_style['row_line'],
                             row_color=theme.value[1])
